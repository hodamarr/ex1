...
function Contacts(props) { 
    ...
    for(let u of listOfName){
        for (let k of props.contacts){
            let x = k.name;
    ...
    return (
        <div style={{ padding: "5px"}}>
            <ul className="nav flex-column" >
                ...
                {props.contacts.map((c, index) => <li key={index} className="nav-item"
                        ...
                        <button className=" btn-secondary" style={{width:"100%", backgroundColor:"transparent", display:"flex", borderColor:"transparent"}}
                                onClick={() => props.onSelectedChat(c.name)}>
                            <ContactItem {...c} time={c.msgs.length> 0 ? c.msgs[c.msgs.length-1].time : "" }/>
                        </button>
                </li>)}
            </ul>
                    ...
                    <div className="modal-content">
                            ...
                            <form>
                                <div className="mb-3">
                                    <select className="form-select" multiple aria-label="multiple select example"
                                        ...
                                        {(listOfName || []).map(i => <option key={i} value={i}>{getNick(i)}</option>)}
                                    </select>
                                </div>
                            </form>
                        </div>
                        <div className="modal-footer">
                            ...
                            <button type="button" className="btn btn-primary"
                                    onClick={() => props.setter({name:getNick(opt), pic:getPic(opt), msgs:[]})} data-bs-dismiss="modal">Done</button>
                        </div>
                    </div>
                </div>
            </div>
